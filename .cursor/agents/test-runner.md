...
**Stack:** Python 3.12 (FastAPI backend) + TypeScript/React 18 (Vite frontend)

### Frontend (работать из папки `frontend/`)
```bash
...
### Backend (работать из папки `backend/` или через Docker)
```bash
# Тесты отсутствуют — рекомендуется настроить pytest
# При наличии pytest:
docker exec uute-backend python -m pytest backend/tests/ -v
...
**Примечание:** Автоматические тесты для backend не настроены. При обнаружении проблем — сообщать пользователю о необходимости добавить pytest.
