...
- Celery hardening (D5, 2026-04-22): `visibility_timeout=3600` (было 86400), `task_reject_on_worker_lost=True`, `task_acks_late=True`. Удалён `apply_async(countdown=86400)` для `info_request` — теперь только Beat `process_due_info_requests` (тик 5 мин). Регрессия защищена `tests/test_celery_hardening.py`. Закрывает фазу D audit-roadmap §3.
