...
- Typed API фронтенда (E1, 2026-04-22): `frontend/src/api/types.ts` и `frontend/src/api/openapi.json` генерируются скриптом `scripts/generate-api-types.sh` (`app.openapi()` → `openapi-typescript`). `frontend/src/api.ts` использует `components['schemas'][…]` — при любом изменении Pydantic-схем нужно перегенерировать клиент, иначе CI-job `api-types-drift` падает. Артефакты коммитятся в репо.
