"""
@file: app/schemas/jsonb/tu.py
@description: Pydantic-модель для `Order.parsed_params` — данные, извлечённые LLM из
    технических условий теплоснабжения. Исторически жила в `services/tu_schema.py`;
    теперь — каноническое место, `services/tu_schema.py` оставлен как backward-compat shim.
@dependencies: pydantic v2.
@created: 2026-04-21 (перенос из services/tu_schema.py)
"""
...
# Базовый конфиг для всех моделей JSONB: игнорируем неизвестные ключи, чтобы
# исторические записи с устаревшими ключами не роняли чтение; см. `order_jsonb`.
class _TuBase(BaseModel):
    model_config = ConfigDict(extra="ignore")


class RSOInfo(_TuBase):
...
class MeteringRequirements(_TuBase):
...
class ConnectionScheme(_TuBase):
    """Схема подключения."""
...
class AdditionalRequirements(_TuBase):
...
def get_missing_fields(data: TUParsedData) -> list[str]:
    ...
    Используется оркестратором для определения — нужно ли запрашивать
    доп. информацию у клиента (`OrderStatus.WAITING_CLIENT_INFO`).
    """
    missing: list[str] = []

    for dotted_path in REQUIRED_PARSED_FIELDS:
        parts = dotted_path.split(".")
        obj: object | None = data
        for part in parts:
