...
- **Шаги выполнения**:
  - [x] Объект `PARAM_TO_SURVEY`, `getNestedValue`, нормализация select-полей
  - [x] `prefillSurvey`: обход маппинга, бейджи «из ТУ» / жёлтые подсказки для производителя и типа расходомера
  - [x] Блок уверенности и предупреждений парсера; `hydrateSurveyFromOrder` — приоритет `survey_data` над ТУ
  - [x] `enableSurveyFields` / `clearSurveyDecorations`
  - [x] Доп. поля: `rso.rso_name`, `metering.heat_calculator_model`, дублирование Ду на обратку, `total_load` → отопление, комментарии из `additional.notes`
...
- **Шаги выполнения**:
  - [x] Запись в docs/changelog.md (итог ревью: ограничение POST /survey по статусу, рефактор upload.html)
  - [x] Обновить docs/tasktracker.md (блок «Умный опросный лист»)
  - [x] Актуализировать docs/project.md
