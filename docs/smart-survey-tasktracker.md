...
- **Шаги выполнения**:
  - [ ] После `/submit` для custom-заказа: вместо `showCompleted()` показать блок ожидания парсинга
  - [ ] Запустить polling `GET /landing/orders/{id}/upload-page` каждые 5 секунд
  - [ ] Когда `order_status` изменится с `tu_parsing` на `tu_parsed` — остановить polling
  - [ ] Извлечь `parsed_params` из ответа и вызвать `prefillSurvey(parsedParams)`
  - [ ] Показать блок `#surveyCard` с разблокированными полями
  - [ ] Для express-заказов — поведение без изменений (showCompleted)
