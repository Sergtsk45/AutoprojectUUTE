...
## Задача: Клиентский конфигуратор схем в режиме демонстрации
- **Статус**: Завершена
- **Описание**: Клиент на `/upload` теперь только выбирает конфигурацию принципиальной схемы и видит SVG-превью. ГОСТ-рамка A3, клиентская PDF-генерация и скачивание PDF убраны из клиентского сценария; PDF-подготовка переносится в будущий инженерный admin-flow.
- **Шаги выполнения**:
  - [x] `/api/v1/schemes/preview` возвращает standalone SVG без `gost_frame_a3`.
  - [x] Публичный `/api/v1/schemes/{order_id}/generate` отключён и возвращает `410 Gone`.
  - [x] В `upload.html`/`scheme.js` убраны кнопка генерации PDF, success/download UI и вызовы generate-endpoint.
  - [x] `heat_scheme` исключён из обязательных клиентских документов, скрытая автогенерация PDF в `process_client_response` отключена.
  - [x] Обновлены тесты preview-only поведения и обязательных документов.
- **Зависимости**: текущий DXF renderer `SchemeType.DEP_SIMPLE`; будущая генерация PDF будет отдельной задачей админки инженера.

## Задача: DXF-шаблон первой принципиальной схемы
- **Статус**: Завершена
- **Описание**: Для первой конфигурации `SchemeType.DEP_SIMPLE` добавлен DXF-шаблонный renderer, который превращает исходник DXF в SVG-фрагмент для клиентского предпросмотра. Остальные 7 схем пока используют legacy programmatic renderer до подготовки исходников DXF/SVG.
- **Шаги выполнения**:
