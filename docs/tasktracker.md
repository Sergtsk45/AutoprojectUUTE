...
## Задача: SVG схемы 6–8 (независимые) — все 8 схем готовы ✅ (2026-04-20)
- **Статус**: Завершена
- **Описание**: Реализованы схемы 6–8 (независимые конфигурации с двумя контурами через теплообменник): схема 6 — базовая независимая (сетевой/внутренний контур, 2-ходовой клапан, насос ВК, подпитка G3); схема 7 — + ГВС (врезка из сетевого контура); схема 8 — + ГВС + вентиляция. Все 8 типовых конфигураций протестированы и работают.
- **Шаги выполнения**:
  - [x] Схема 6: `render_scheme_06_indep` — два контура через теплообменник, 2-ходовой клапан, насос ВК, подпитка G3
  - [x] Схема 7: `render_scheme_07_indep_gwp` — схема 6 + блок ГВС из сетевого контура
  - [x] Схема 8: `render_scheme_08_indep_gwp_vent` — схема 7 + ветка вентиляции
  - [x] Тесты всех схем (5/5 checks каждая)
  - [x] Обновить docs (changelog, tasktracker, roadmap — задача 3 ✅)
- **Зависимости**: схемы 1-7, библиотека элементов
- **Результат**: Все 8 типовых конфигураций реализованы. Задача 3 (SVG-рендер схем) полностью завершена.

## Задача: SVG схемы 3–5 (зависимые с клапаном/ГВС/вентиляцией) (2026-04-20)
- **Статус**: Завершена
- **Описание**: Реализованы схемы 3–5 (все зависимые варианты): схема 3 — трёхходовой клапан и насос на перемычке; схема 4 — комбинация клапана/насоса и ГВС; схема 5 — клапан/насос, ГВС и параллельная ветка вентиляции. Все схемы проверены (5/5 checks).
- **Шаги выполнения**:
  - [x] Схема 3: `render_scheme_03_dep_valve` — трёхходовой клапан перед G1, перемычка с насосом между подачей и обраткой
  - [x] Схема 4: `render_scheme_04_dep_valve_gwp` — топология схемы 3 + блок ГВС (как в схеме 2)
  - [x] Схема 5: `render_scheme_05_dep_valve_gwp_vent` — схема 4 + ветка вентиляции (врезка от подачи, теплообменник, насос, возврат в обратку, зона вентиляции)
  - [x] Тесты всех схем: проверка подписей, элементов, зон
  - [x] Обновить docs (changelog, tasktracker, roadmap)
- **Зависимости**: схемы 1-2, библиотека элементов

## Задача: SVG схема 2 (зависимая с ГВС) и fix text_label (2026-04-20)
- **Статус**: Завершена
- **Описание**: Реализована схема 2 — базовая топология + блок ГВС справа: теплообменники 1-й (от обратки) и 2-й (от подачи) ступени, врезки в магистрали, насос ГВС, расходомер G3, подписи ХВС/ГВС/подогревателей, пунктирная зона ГВС. Исправлена критическая ошибка в `text_label()` (многострочный return без скобок возвращал пустую строку).
- **Шаги выполнения**:
  - [x] Реализация `render_scheme_02_dep_simple_gwp(params)`
  - [x] Топология ГВС: врезки в подачу/обратку, 2 теплообменника, насос, G3
  - [x] Исправление `text_label()` — обёртка return в скобки
  - [x] Ручная проверка SVG: все подписи присутствуют (6/6 checks)
  - [x] Обновить docs (changelog, tasktracker, roadmap)
- **Зависимости**: схема 1, библиотека элементов

## Задача: SVG рендерер схем — эталонная схема 1 (2026-04-20)
- **Статус**: Завершена
- **Описание**: Создан модуль `scheme_svg_renderer.py` с диспетчером и эталонной схемой 1 (зависимая без клапана): подача/обратка, задвижки, фильтры, датчики температуры/давления, расходомеры G1/G2, радиатор и тепловычислитель УУТЭ с таблицей параметров. Топология проверена сборкой с ГОСТ-рамкой (SVG 18 КБ).
- **Шаги выполнения**:
  - [x] Каркас модуля с диспетчером `render_scheme()`
  - [x] Реализация `render_scheme_01_dep_simple(params)` — топология, размещение элементов
  - [x] Ручная проверка SVG (сохранение в /tmp/scheme_full.svg, просмотр в браузере)
  - [x] Обновить `docs/changelog.md`, `docs/tasktracker.md`, `docs/scheme-generator-roadmap.md`
- **Зависимости**: задачи 2.1, 2.2 (библиотека элементов и ГОСТ-рамка)
