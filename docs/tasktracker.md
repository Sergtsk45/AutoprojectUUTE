...
## Задача: UI конфигуратора схем в upload.html ✅ (2026-04-23)
- **Статус**: Завершена
- **Описание**: Реализован полнофункциональный UI конфигуратора принципиальных схем на странице загрузки документов. Клиент отвечает на 4 вопроса (тип присоединения, клапан, ГВС, вентиляция), видит превью SVG в браузере и генерирует PDF по нажатию кнопки. Схема автоматически сохраняется как OrderFile и отмечается в чеклисте документов. Реализовано предзаполнение из parsed_params и восстановление состояния из survey_data.
- **Шаги выполнения**:
  - [x] Изучить структуру upload.html, стили и JavaScript
  - [x] Добавить HTML-секцию конфигуратора между `surveyCard` и `requiredDocsCard`
  - [x] Реализовать CSS-стили (радиокнопки, чекбоксы, превью-контейнер, success/error сообщения)
  - [x] Реализовать логику выбора конфигурации (условное отображение вопросов, валидация)
  - [x] Реализовать загрузку превью SVG через `POST /schemes/preview` с автообновлением
  - [x] Реализовать генерацию PDF через `POST /schemes/{order_id}/generate` и отображение статуса
  - [x] Добавить предзаполнение из `parsed_params` (тип присоединения, ГВС из ТУ)
  - [x] Добавить восстановление конфигурации из `survey_data.scheme_config`
  - [x] Интегрировать вызовы `showSchemeConfiguratorIfNeeded()` после сохранения опроса и в `init()`
  - [x] Проверить синтаксис HTML/CSS/JS (2997 строк, div-теги сбалансированы)
  - [x] Обновить документацию (changelog, tasktracker, roadmap)
- **Зависимости**: задача 5 (API эндпоинты)
- **Результат**: Полный UI конфигуратора готов. Задача 6 завершена. Можно переходить к задаче 7 (интеграция с админкой и пайплайном).

## Задача: API эндпоинты конфигуратора схем ✅ (2026-04-23)
- **Статус**: Завершена
- **Описание**: Реализованы REST API эндпоинты для конфигуратора принципиальных схем. Клиент может получить список доступных конфигураций, сгенерировать превью SVG в браузере, и создать PDF схемы, который автоматически сохраняется как файл заявки. Конфигурация схемы записывается в survey_data для последующего восстановления.
- **Шаги выполнения**:
  - [x] Изучить структуру API, модели Order/OrderFile, OrderService
  - [x] GET /api/v1/schemes/templates — список 8 конфигураций с описаниями
  - [x] POST /api/v1/schemes/preview — превью SVG (SchemeConfig → SVG с ГОСТ-рамкой)
  - [x] POST /api/v1/schemes/{order_id}/generate — генерация PDF, сохранение OrderFile, запись в survey_data
  - [x] GET /api/v1/schemes/{order_id}/config — получение сохраненной конфигурации
  - [x] Подключить роутер `scheme_generator_router` в `main.py`
  - [x] Проверка импортов и линтера (без ошибок)
  - [x] Обновить документацию (changelog, tasktracker, roadmap)
- **Зависимости**: задача 4 (PDF-рендер), задача 3 (SVG-рендер), задача 1 (Pydantic-модели)
- **Результат**: Полный API готов. Задача 5 завершена. Можно переходить к задаче 6 (UI конфигуратора в upload.html).

## Задача: PDF-рендер схем — WeasyPrint интеграция ✅ (2026-04-23)
- **Статус**: Завершена
- **Описание**: Реализован модуль генерации PDF из SVG-схем через WeasyPrint. Созданы HTML-шаблоны для форматов A3 (альбомная) и A4 (книжная). SVG с ГОСТ-рамкой оборачивается в HTML и конвертируется в PDF. Установлены совместимые версии зависимостей (weasyprint 61.2, pydyf 0.10.0). Протестирована генерация PDF для всех конфигураций.
- **Шаги выполнения**:
  - [x] Установить `weasyprint` и `pydyf` в `requirements.txt` (совместимые версии: 61.2 и 0.10.0)
  - [x] Создать HTML-шаблоны `gost_frame_a3.html` и `gost_frame_a4.html` (Jinja2, точные размеры страниц)
  - [x] Создать `scheme_pdf_renderer.py` с функциями `render_scheme_pdf()` и `render_scheme_pdf_from_params()`
  - [x] Расширить `SchemeParams` — добавить метки датчиков и параметры таблицы УУТЭ
  - [x] Протестировать генерацию PDF (A3 и A4) — созданы тестовые файлы `test_scheme_a3.pdf` и `test_scheme_a4.pdf`
  - [x] Обновить документацию (`changelog.md`, `tasktracker.md`, `scheme-generator-roadmap.md`)
- **Зависимости**: задача 3 (SVG-рендер схем), задача 2 (ГОСТ-рамка)
- **Результат**: Полный пайплайн генерации PDF готов. Задача 4 (ГОСТ-рамка и PDF-рендер) завершена. Можно переходить к задаче 5 (API эндпоинты).
